...
TEST_CASE("every asset group reaches a tab, categorised or not", "[assets][tabs]") {
    // THE failure this design could introduce. Groups are dealt into tabs by a
    // string the application sets, so the way to lose a chooser is to forget
    // that string -- and the panel would then be the reason a capability is
    // unreachable, which is worse than the flat list it replaced.
    //
    // `toyAssets()` deliberately sets no category at all, so this also pins
    // the fallback: two groups in, two pickers out.
    using mh::foundation::AssetGroup;
    std::vector<AssetGroup> groups = toyAssets();
    AssetGroup filed{"Hair", {{"none", "None"}, {"/h/afro.mhclo", "Afro"}}, 0};
    filed.category = "Geometries";
    groups.push_back(filed);

    mh::ui::AssetPanel panel(groups);
    // Recursive by default, so it counts pickers inside the tab pages.
    CHECK(panel.findChildren<QComboBox*>().size() == 3);
    // And each is still addressable by its group name, which is what every
    // other caller in the application uses.
    CHECK(panel.choice(QStringLiteral("Hair")) == QStringLiteral("none"));
    CHECK(panel.choice(QStringLiteral("Skin")) == QStringLiteral("/l/caucasian.png"));
}

TEST_CASE("the asset tabs follow the reference's category order", "[assets][tabs]") {
    // The order is not a preference. `core/mhmain.py:523-527` creates its
    // categories as Modelling, Geometries, Materials, Pose/Animate, Rendering;
    // Modelling is the slider dock in this port, so the Assets panel carries
    // the remaining four in that sequence.
    //
    // Deliberately built in the WRONG order to prove the panel sorts rather
    // than echoing whatever order the application happened to use.
    using mh::foundation::AssetGroup;
    std::vector<AssetGroup> groups;
    for (const auto& [name, category] :
         std::vector<std::pair<const char*, const char*>>{{"Scene lighting", "Rendering"},
                                                          {"Pose", "Pose/Animate"},
                                                          {"Skin material", "Materials"},
                                                          {"Hair", "Geometries"}}) {
        AssetGroup group{name, {{"none", "None"}}, 0};
        group.category = category;
        groups.push_back(group);
    }

    mh::ui::AssetPanel panel(groups);
    auto* tabs = panel.findChild<QTabWidget*>(QStringLiteral("assets.tabs"));
    REQUIRE(tabs != nullptr);
    REQUIRE(tabs->count() == 4);
    CHECK(tabs->tabText(0) == QStringLiteral("Geometries"));
    CHECK(tabs->tabText(1) == QStringLiteral("Materials"));
    CHECK(tabs->tabText(2) == QStringLiteral("Pose/Animate"));
    CHECK(tabs->tabText(3) == QStringLiteral("Rendering"));
}

TEST_CASE("a category nothing uses gets no tab", "[assets][tabs]") {
    // An empty tab reads as a broken panel, so a category in the order list
    // that no group claims must not be created. Only Geometries is used here.
    using mh::foundation::AssetGroup;
    AssetGroup only{"Hair", {{"none", "None"}}, 0};
    only.category = "Geometries";
    const std::vector<AssetGroup> groups{only};

    mh::ui::AssetPanel panel(groups);
    auto* tabs = panel.findChild<QTabWidget*>(QStringLiteral("assets.tabs"));
    REQUIRE(tabs != nullptr);
    INFO("tabs: " << tabs->count());
    CHECK(tabs->count() == 1);
    CHECK(tabs->tabText(0) == QStringLiteral("Geometries"));
}
