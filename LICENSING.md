...
| **libktx** (KTX-Software) | v4.4.2 = commit `4d6fc70eaf62ad0558e63e8d97eb9766118327a6` (pinned to the SHA, never the tag alone) | Apache-2.0 | `Apache-2.0` | **KTX2 / ETC1S + BasisLZ texture compression** for glTF (`KHR_texture_basisu`). Linked into `mh_io`, and **OPTIONAL on the same terms as draco and assimp**, so a build without it writes uncompressed textures rather than failing to configure. **Not in Homebrew** — verified 2026-09-20: `brew info ktx` reports *No available formula*, and no `libktx*` exists on this machine — so it is fetched at the pinned SHA rather than found. Transitively Apache-2.0 throughout the parts we link: its in-tree `external/basisu`, `external/dfdutils` and `external/astc-encoder` are all Apache-2.0; `zstd` resolves to the **system** library (BSD-3-Clause) via its `cmake/modules/Findzstd.cmake`. `fmt`, `lodepng`, `cxxopts` and `SDL_gesture` build only the tools and viewer, which we do not build. **ONE FILE IS EXCLUDED AND THE EXCLUSION IS LOAD-BEARING:** `external/etcdec/etcdec.cxx` is the Ericsson Texture Compression Codec SLA (`LicenseRef-ETCSLA`) — **field-of-use restricted**, which §5.2 forbids outright, and upstream says so itself (*"The file lib/etcdec.cxx is not open source"*). **`-DKTX_FEATURE_ETC_UNPACK=OFF` keeps it out of the build**; the option defaults **ON**, so it must be passed explicitly. Verified in the v4.4.2 source (`CMakeLists.txt:47`, `:435-438`, `:572`; `lib/etcunpack.cxx` guards its whole body, lines 29–277) and in a real build, where `etcdec` appears **0 times**. A gate asserts this against the built artefact rather than against this table. Licence texts and the full reasoning are in `third_party/licenses/`. Measured on this machine: configure 2 s, build 7 s, `libktx.a` 3,962,680 bytes. |
...
|---|---|---|---|---|
| **Python** (`find_package(Python3 QUIET COMPONENTS Interpreter)`) | 3.14.6 | PSF-2.0 | `PSF-2.0` | A CONFIGURE-time dependency of the TEST tree only, and an optional one. `tests/CMakeLists.txt` looks for an interpreter so that `hair_assets_are_generated` can run `tools/make_hair_styles.py --check`, which fails if `data/hair/afro.*` or `data/hair/cornrows.*` stops matching a fresh derivation -- without it "generated" is a claim this file makes with nothing enforcing it. QUIET and guarded on `Python3_Interpreter_FOUND`: a machine without an interpreter simply does not register that one test, rather than failing to configure. Never linked, never redistributed, and no Python ships in the application. |
| Pillow | 12.3.0 (unpinned) | HPND (MIT-like) | `HPND` | Three tools. `tools/make_appicon.py` masks the brand logo into Apple's squircle geometry and emits the `.iconset`; `tools/make_eyes.py` recolours the shipped iris into the six eye colours and, with `--check`, verifies they still match the generator; `tools/make_helper_proxies.py` re-tints `skinmat_caucasian.png` into one matcap per helper-cage slot (teeth, tongue, hair) and has the same `--check`. The committed `AppIcon.icns`, `*_eye.png` and `data/*/skinmat_*.png` are what the build consumes, so a machine without Pillow builds fine. Installed in `.venv-mh` locally and **pip-installed in the `inventories` CI job** since 2026-09-07, so that `--check` can run there. Never in the application. |
| NumPy | 2.5.2 local / 2.5.3 CI (unpinned) | BSD-3-Clause | `BSD-3-Clause` | The array maths behind `tools/make_eyes.py`'s iris mask and behind `tools/make_helper_proxies.py`'s luminance re-tint, plus both staleness comparisons. Same standing as Pillow: a developer and CI tool, never linked, never redistributed, and unpinned because pinning a dev tool is churn for no safety. The honest trade: if a future NumPy or Pillow changes rounding, the gate reports the committed assets as stale when they are not. That failure is loud and harmless, and the fix would be to pin at that point. Verified 2026-09-07 that 2.5.2 and 2.5.3 both agree with the committed PNGs. |
...
| **basisu** (`basis_universal`) | 2.50 (Homebrew bottle, installed 2026-09-19 on the owner's instruction) | Apache-2.0 | `Apache-2.0` | **ORACLE ONLY**, out-of-process, the same standing as Blender and Maya. Two uses, both test-time: `basisu in.png -ktx2` produces a REFERENCE KTX2 to compare our own writer against, and `basisu -unpack ours.ktx2` runs the upstream transcoder over what we wrote, which is how "our KTX2 is conformant" stops being a claim and becomes a measurement. It is the same role `legacy/python` plays for the port. **CLI only, and that is verified rather than assumed**: the bottle installs one 3.4 MB executable and `find` over the Cellar returns no `.a`, no `.dylib` and no headers, so it *cannot* be linked even by accident. Never linked, never a build dependency, never redistributed, and no CI job installs it — the KTX2 writer it validates is our own code, written from the Khronos specification, precisely so that nothing has to be fetched or built in CI. |
