...
| `data/teeth/*`, `data/tongue/*`, `data/hair/*` (generated) | **CC0-1.0**, inherited | `tools/make_helper_proxies.py` derives each proxy's geometry from `data/3dobjs/base.obj`'s own `helper-*` groups and each matcap from `data/litspheres/skinmat_caucasian.png`. Both sources are CC0-1.0 bundled assets (row above), so the derivatives are too. Nothing here is authored from an outside asset, and nothing is MetaHuman-derived. |
