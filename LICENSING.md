...
|---|---|---|---|---|
| **Python** (`find_package(Python3 QUIET COMPONENTS Interpreter)`) | 3.14.6 | PSF-2.0 | `PSF-2.0` | A CONFIGURE-time dependency of the TEST tree only, and an optional one. `tests/CMakeLists.txt` looks for an interpreter so that `afro_assets_are_generated` can run `tools/make_hair_styles.py --check`, which fails if `data/hair/afro.*` stops matching a fresh derivation -- without it "generated" is a claim this file makes with nothing enforcing it. QUIET and guarded on `Python3_Interpreter_FOUND`: a machine without an interpreter simply does not register that one test, rather than failing to configure. Never linked, never redistributed, and no Python ships in the application. |
