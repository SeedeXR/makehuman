...
/// Enamel, generated by tools/make_teeth.py from the skin matcap's luminance so
/// the teeth are lit from the same direction as the face around them.
///
/// It lives beside the teeth rather than in data/litspheres, which is what the
/// body-skin chooser enumerates.
std::filesystem::path teethLitsphere() {
    return dataDir() / "teeth" / "skinmat_teeth.png";
}
...
std::vector<mh::foundation::AssetGroup> buildAssetGroups(
    ...
    const std::string& currentEyeColour, const std::string& currentTeeth) {
    ...
    // Teeth: the second proxy chooser, and the first filled under directive
    // 13.5 ("if there are no assets or data from legacy, procedurally generate
    // and wire through"). Legacy ships nothing here; `tools/make_teeth.py`
    // builds the proxy from the base mesh's OWN `helper-*-teeth` cage geometry,
    // so the shape is the reference's rather than invented.
    mh::foundation::AssetGroup teeth;
    teeth.name = "Teeth";
    teeth.choices.push_back({kNoProxy, "None"});
    teeth.selected = 0;
    for (const fs::path& p : filesWithExtension(dataDir() / "teeth", ".mhclo")) {
        teeth.choices.push_back({p.string(), prettyName(p, "")});
        if (p.stem().string() == currentTeeth) {
            teeth.selected = static_cast<int>(teeth.choices.size()) - 1;
        }
    }
    groups.push_back(std::move(teeth));
...
constexpr const char* kTeethSaveName = "teeth";
...
int main(int argc, char** argv) {
    ...
    const QCommandLineOption teethOpt(
        QStringLiteral("teeth"),
        QStringLiteral("Teeth proxy to wear: none (default), or the stem of a .mhclo under "
                       "data/teeth"),
    ...
    // The teeth slot takes the same shape as the eyes, including the `.mhm`
    // round trip: `recordProxy` and `proxyFromDocument` are keyed by slot name,
    // so a second slot costs one constant and no new machinery.
    std::string teethChoice = parser.value(teethOpt).toStdString();
    if (!parser.isSet(teethOpt)) {
        if (const auto fromDoc = proxyFromDocument(document, kTeethSaveName)) {
            teethChoice = *fromDoc;
        }
    }
    ...
    const auto assetGroups =
        buildAssetGroups(parser.value(poseOpt).toStdString(), litsphereChoice, eyesChoice,
                         skinMaterialRef(), rigNameRef(), eyeColourRef(), teethChoice);
    ...
    // Teeth get their own matcap for the same reason the eyes do. Wiring them to
    // the body's skin litsphere was tried first and RENDERED: the teeth came out
    // at RGB (233, 155, 123), the same orange as the lip in front of them,
    // because the teeth material carries no diffuse texture and the litsphere
    // shader multiplies the matcap by the texture alone (litsphere.frag:157-162).
    // A fixed matcap also means a worn set does not go stale when the Litsphere
    // picker changes `skin` under it.
    if (const std::string teeth = selectedChoice(assetGroups, "Teeth");
        !teeth.empty() && teeth != kNoProxy) {
        if (auto worn = wearProxy(teeth, *mesh, teethLitsphere())) {
            wornProxies.insert_or_assign(QStringLiteral("Teeth"), std::move(*worn));
        }
    }
    ...
    if (parser.isSet(saveOpt)) {
        ...
        // Same shape for the teeth, and it is what makes `proxyFromDocument`
        // above reachable: without this line a character saved wearing teeth
        // reopened without them and the record was silently lost.
        const auto* teethWorn = wornProxies.count(QStringLiteral("Teeth")) != 0
                                    ? &wornProxies.at(QStringLiteral("Teeth")).proxy
                                    : nullptr;
        recordProxy(doc, kTeethSaveName, teethWorn != nullptr ? teethWorn->name : std::string{},
                    teethWorn != nullptr ? teethWorn->uuid : std::string{});
    ...
    const auto applyChoice = [&](const QString& group, const QString& id) {
        ...
        if (group == QLatin1String("Teeth")) {
            ...
            } else if (auto worn = wearProxy(id.toStdString(), *mesh, teethLitsphere())) {
