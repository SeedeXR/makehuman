...
- [x] **`TMatrix` shear is implemented** (2026-09-08). All nine spellings
      (`shear_*`, `l_shear_*`, `r_shear_*`) parse and reach the fit, so a
      `.mhclo` MakeHuman reads is no longer turned away.
      - **The item's premise was wrong, and measuring the reference is what
        showed it.** It said implementing "needs a general SVD-based affine
        solve". It does not: `matrixFromShear` feeds
        `affine_matrix_from_points` two **axis-aligned** boxes — source corners
        built per axis from the two authored coordinates, target corners from
        one component of two base vertices — and the exact map between two
        axis-aligned boxes is DIAGONAL. Measured off-diagonal: **4.6e-15**,
        float noise. MakeHuman's "shear" cannot express shear.
      - What it CAN express that `x_scale` cannot is a **signed** ratio: the
        scale form takes an absolute difference, so it can never mirror an axis.
      - **A zero authored span yields 0, not infinity** — the least-squares
        solve collapses that axis. The obvious ratio would have produced inf and
        a mesh of NaNs. Captured as a fixture case for exactly that reason.
      - A **partial** spec is refused: `matrixFromShear` unpacks all three axes
        unconditionally, so the reference raises on a file naming one. Refusing
        beats fitting with two silently identity axes.
      - Fixture generated by the oracle (`tools/capture_fixture.py shear`), the
        same call as `.mhpose`: "untestable machinery" stops applying the moment
        the reference can be asked for the answer.
      - Six mutations caught, including the one this chunk actually shipped: a
        `key.size() > 7` guard that passed the sided spellings and silently
        dropped the unsided ones, because `shear_x` is exactly seven characters.
...
- [ ] **OWNER REQUEST (2026-09-05): complete the UI to match the reference.**
      **>>> THIS IS THE NEXT CHUNK, ahead of any more hair-style work. <<<**
      ...
      anything.
      **AnimationLibrary is BLOCKED ON CONTENT, not on a tab -- corrected
      2026-09-13 after measuring, and the earlier note below was wrong to imply
      it was ready.** The shipped animations drive NOTHING: `walk1.bvh` names
      75 joints of the OLD MakeHuman skeleton (`Spine1`, `Clavicle_L`) and
      matches **0** of the default rig's 163 or the application's
      `mixamo_superset` 179. Every frame therefore poses identically, which is
      why `--pose-frame` looks broken on them and works perfectly on
      `face-poseunits.bvh` (163 of 163 matched, 60 frames).
      `tests/unit/test_pose_frame.cpp` had ALREADY recorded this in its header;
      the measurement here reproduced it independently rather than finding it.
      `data/rigs/mixamo_retarget.json` does not rescue them either: it is 65
      `Mixamo bone -> superset bone` pairs and only **5** of walk1's 75 joints
      appear as keys, so the walks are in a THIRD naming convention. There is
      no C++ loader for it in any case.
      **OWNER DECIDED, 2026-09-13:** *"for rigs we can have 2 alternatives
      mixamo and old make human names hence one can choose btn either of the 2
      rigs whilst exporting"* -- so the rig's bone NAMING becomes a choice the
      user makes at EXPORT time: Mixamo names or old-MakeHuman names.
      What already exists, measured:
      * `data/rigs/default.mhskel` -- 163 bones, THIS port's names
        (`clavicle.L`, `spine05`).
      * `data/rigs/mixamo_superset.mhskel` -- 179 bones, the application's
        default rig.
      * `data/rigs/mixamo_retarget.json` -- 65 `Mixamo bone -> superset bone`
        pairs, generated by `tools/mixamo_mapping.py`. **No C++ loader reads
        it** (grep: zero hits in `src/`).
      * `data/animations/**/*.bvh` -- 75 joints in the OLD MakeHuman convention
        (`Root`, `Spine1`, `Clavicle_L`, `UpArm_L`), a concrete shipped sample
        of the second naming set.
      What that implies for the work:
      1. A loader for `mixamo_retarget.json` -- the data is there and unread.
      2. A second mapping for the old-MakeHuman names. The 75 joints in
        `walk1.bvh` are the authoritative list of what has to be covered, and
        exactly 5 of them appear in the Mixamo table, so it is a genuinely
        separate table rather than a variation.
      3. An export-time selector, e.g. `--rig-names mixamo|makehuman-legacy`,
        renaming bones on the way out of every skeleton-carrying format
        (BVH, glTF/GLB, FBX, DAE, USD).
      **CONFIRMED by the owner, 2026-09-13, both questions asked and answered:**
      * **THREE naming options, not two**: Mixamo, MakeHuman 1.x
        (`Spine1`/`Clavicle_L`, what the shipped walks use) and this port's own
        names (`clavicle.L`/`spine05`). So the MakeHuman 1.x table in item 2 IS
        needed, and `walk1.bvh`'s 75 joints are the list it must cover.
      * **The mapping applies on IMPORT as well as export.** That is the answer
        that matters: renaming on the way IN makes `walk1`, `zombieWalk1` and
        `dance1` actually drive the rig, so **`AnimationLibrary` stops being
        blocked on content**. Move it out of `blocked` only once a walk
        measurably poses the character -- the gate for that already exists and
        is honest: `bonesDrivenBy` must stop returning 0, and
        `tests/unit/test_pose_frame.cpp`'s identity test is written to FAIL the
      ...
      **The next buildable view is one of the other four**: `BackgroundChooser`,
      ...
      The superseded note follows, kept because its API citations are correct:
...
- [~] **`ExpressionTaskView` / `ExpressionMixerTaskView` — the ENGINE is done,
      the chooser is still content-blocked.** `--expression <file.mhpose>`
      applies an expression end to end (2026-09-08): load the file, build the 60
      face units, blend the named ones at their weights, and `mixPoses` the
      result over whatever `--pose` gave, for exactly the bones the blend moves.
      - **The loader had no caller.** It landed with oracle parity the chunk
        before and nothing in the shipped binary used it, so the format was
        still unusable — machinery with a test, which is only half of what the
        original objection was about.
      - **The bone list is derived, not configured**: every bone the blend
        leaves non-identity. Measured on the fixture: 34 bones, all of them
        `jaw`/`oris*`/`levator*`/`risorius*`/`tongue*` — no root, no spine, so
        an expression cannot drag the body.
      - Rendered two of them and looked. The fixture's five overlapping units at
        0.2–0.9 give an extreme funnel of a mouth, which is what an ADDITIVE
        blend of that input should give; a mild file (four units at 0.25–0.4)
        gives a faint asymmetric half-smile. 95% of the changed pixels sit in
        the head band and exactly **4** stray elsewhere — MSAA edge noise, and a
        reminder that a bounding box is a terrible summary statistic: those 4
        made the bbox span the whole body.
      - Twelve app gates, and the twelfth exists because a mutation survived
        eleven: applying every unit at FULL strength still moved the face, still
        layered, still differed from every other export. `expression_half_weights
        .mhpose` — the same five units at half strength — is what pins the
        weights.
      - Still blocked: **zero `.mhpose` files ship**, so a chooser would have an
        empty list. And the two task views are different systems — the mixer
        drives this face rig, while `buildExpressionBlendshapes` uses MORPH
        TARGETS (`data/targets/expression/units/`, 34 units × 3 ethnicities).
        **Which the port should offer is an owner decision, not a chunk.**
- [x] **Eye colours (owner request, 2026-09-06).** Six: `brown` as shipped plus
      `amber`, `hazel`, `green`, `blue`, `grey`, generated by
      `tools/make_eyes.py` from the CC0 source by hue-shifting the iris only.
      The mask is measured, not eyeballed: at saturation > 0.35 there are 79,164
      pixels and **zero** outside the two iris discs, so the sclera's blood
      vessels are untouched by construction — and `--check` re-verifies that on
      every run, because a future retouch of the source could break it silently.
      Luminance is preserved per pixel so the fibre structure survives; a flat
      fill gives a plastic disc.
      - `--eye-colour`, an "Eye colour" picker, and an `eyeMaterial` line in the
        `.mhm`. Independent of the Eyes group, which picks the GEOMETRY — both
...
- [x] **OWNER REQUEST (2026-09-06): test the PRODUCTION build, not only debug.**
      ...
      **release-preset `.app` bundle** is run end to end
      ...
      rather than only its unit tests. Worth knowing what that does and does not
      cover:
...
- [x] **`tests/golden/**` is un-ignored now.** The asset-extension rules at the
      top of `.gitignore` (`*.obj`, `*.mhpose`, `*.mhclo`, …) applied to the
      fixture tree as well, and `git add -A` says nothing about an ignored file.
      `tests/golden/mhpose/smirk.mhpose` went missing from its own commit: the
      `.bin` and `cases.json` were tracked, the file the loader parses was not,
