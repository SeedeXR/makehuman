...
FACE_POSEUNITS_BVH = "data/poseunits/face-poseunits.bvh"
FACE_POSEUNITS_JSON = "data/poseunits/face-poseunits.json"
...
def capture_bvh() -> None:
    ...
    for src, stem in (("data/poses/tpose.bvh", "tpose"),
                      (FACE_POSEUNITS_BVH, "faceposeunits")):
        b = bvh_mod.load(src)
        joints = b.getJoints()
...
def _face_pose_units():
    ...
    Shared by the two captures that need them. It was copied between the two,
    which SonarQube flagged as duplication and as a repeated literal -- both
    symptoms of the same thing.

    `allowTranslation="none"` is the reference's own call at this site: it drops
    the BVH's position channels when building pose units. Ours can now carry a
    translation through a blend (M9), which is a deliberate divergence and not
    something this fixture exercises -- every position channel in the shipped
    file is zero anyway.
    """
    import animation
    import bvh as bvh_mod
    ...
    bvh_file = bvh_mod.load(FACE_POSEUNITS_BVH, allowTranslation="none")
    track = bvh_file.createAnimationTrack(skel, name="face-poseunits")
    names = json.loads((REPO / FACE_POSEUNITS_JSON).read_text(encoding="utf-8"))["framemapping"]
    if len(names) != bvh_file.frameCount:
        raise RuntimeError("framemapping/frame count mismatch")
    return skel, animation.PoseUnit(track.name, track._data, names), names, bvh_file.frameCount


def capture_mhpose() -> None:
    """A real `.mhpose` file, written and read back by the REFERENCE.

    `.mhpose` was filed as unimplementable for want of an asset: none ships, and
    `find . -name '*.mhpose'` returns nothing. But the format is fully defined by
    two pieces of the reference we have -- the expression mixer's writer
    (`plugins/7_expression_mixer.py:221-238`, a plain `json.dump` of name,
    description, tags, `unit_poses` and the licence fields) and the parser
    (`shared/animation.py:286-309`, `Pose.fromPoseUnit`). So the fixture is
    GENERATED by running the reference's own serialisation and then blended by
    the reference's own `getBlendedPose`, which is what makes this parity rather
    than machinery.

    `unit_poses` is a dict of unit name -> weight. Note that a dict has no
    guaranteed order in the file, and the blend IS order-dependent: the
    reference feeds `list(dict.keys())` to `getBlendedPose`, so the order is
    whatever the JSON object held. That is captured here rather than assumed --
    the expected blend below is computed from the same order our loader must
    preserve.
    """
    print("capturing: mhpose")
    skel, unit, _names, _frames = _face_pose_units()

    out = GOLDEN / "mhpose"
    out.mkdir(parents=True, exist_ok=True)

    # Written exactly as `saveCurrentPose` writes it, including the fields our
    # loader ignores -- a reader that trips over an unknown key would fail on
    # every file the reference has ever produced.
    # Units chosen to OVERLAP on the same bones -- jaw and lips -- so the
    # multiplication order actually changes the answer. Disjoint units (a brow
    # and a jaw) would blend the same in any order, which would make the
    # ordering contract untestable through the blend. The delta between this
    # order and the sorted one is reported below; if it were ~0 these are the
    # wrong units.
    unit_poses = {
        "JawDrop": 0.9,
        "LipsKiss": 0.7,
        "UpperLipForward": 0.5,
        "lowerLipBackward": 0.6,
        "JawDropStretched": 0.4,
    }
    doc = {
        "name": "Fixture Smirk",
        "description": "Captured from the reference for parity, not shipped as content.",
        "tags": ["fixture", "parity"],
        "unit_poses": unit_poses,
        "author": "MakeHuman C++ port fixture capture",
        "copyright": "CC0",
        "license": "CC0",
        "homepage": "",
    }
    (out / "smirk.mhpose").write_text(json.dumps(doc, indent=4), encoding="utf-8")

    # ...and blended by the reference, in the order the file's dict holds.
    blended = unit.getBlendedPose(list(unit_poses.keys()), list(unit_poses.values()),
                                  only_data=True)
    # The same units in the order a JSON reader that sorts its keys would
    # produce. Recorded so the fixture can PROVE that file order is a contract
    # rather than asserting it on faith.
    sorted_names = sorted(unit_poses.keys())
    sorted_blend = unit.getBlendedPose(sorted_names, [unit_poses[n] for n in sorted_names],
                                       only_data=True)
    order_delta = float(np.abs(np.asarray(blended) - np.asarray(sorted_blend)).max())
    print(f"  file order vs sorted order differs by {order_delta}")
    ...
    (out / CASES_JSON).write_text(json.dumps({
        ...
        "tags": doc["tags"],
        "unit_names": list(unit_poses.keys()),
        "unit_weights": list(unit_poses.values()),
        "bone_count": int(skel.getBoneCount()),
        "sorted_order_differs_by": order_delta,
    }, indent=2))
    _finish("mhpose", entries, {
        "source": "written by the reference's own saveCurrentPose shape, blended by "
                  "PoseUnit.getBlendedPose",
        "units_referenced": len(unit_poses),
        "bones": int(skel.getBoneCount()),
        "note": "no .mhpose ships with MakeHuman; this one is generated so the "
                "loader has a real file, in the reference's own format, to parse.",
    })
...
def capture_poseunits() -> None:
    ...
    skel, unit, names, frame_count = _face_pose_units()
